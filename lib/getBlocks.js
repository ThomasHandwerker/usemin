...
module.exports = function (src, content, removeLivereload) {
	...
	lines.forEach(function (l) {
		...
		if (build) {
			...
			if (isComplexDestination(build[3], complexDestRegEx)) {
				block['dest'] = extractDestination(build[3], complexDestRegEx);
				block['destAlias'] = build[3];
			}
...
function isComplexDestination(dest, regEx) {
...
function extractDestination(dest, regEx) {
	var destParts = dest.match(regEx);
