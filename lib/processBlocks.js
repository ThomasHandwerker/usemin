...
module.exports = function (blocks, destDir, config) {
	blocks.forEach(function (block) {
		...
		if (block.type === 'js') {
			...
			var outSourceMap = blockFileName + '.map';

			if (config.uglifyjs.outSourceMap !== null) {
				var mappath = path.join(destDir, path.dirname(block.dest),
					(isBoolean(config.uglifyjs.outSourceMap)) ? outSourceMap : config.uglifyjs.outSourceMap);
